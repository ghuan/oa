...
public class MailAction extends BaseAction {
	...
	public String get() {
		...
		if (mail.getMailStatus() != 1) {// 假如不为草稿
	...
	public String save() {
		if (mail.getMailStatus() == IS_MAIL) {
			...
			if (sendMessage != null && sendMessage.equals("on")) {
				shortMessageService.save(ContextUtil.getCurrentUserId(), mail.getRecipientIDs(), "您有新邮件!",ShortMessage.MSG_TYPE_SYS);
			}
			// 发送邮件
	...
	public String search() {
		...
		for (MailBox mailBoxTemp : list) {
			...
			buff.append("{boxId:'")
				...
				.append("',subject:'").append(gson.toJson(mailTemp.getSubject()).replace("\"", ""))
				.append("',recipientNames:'")
