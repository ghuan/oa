...
JobChangeForm = Ext.extend(Ext.Panel, {
	...
	initComponents : function() {
		...
		Ext.getCmp('jobChange.newDepNameTree').on('click',function(node){
		     Ext.getCmp('JobChangeForm.newJobName').getStore().removeAll();
		     Ext.getCmp('JobChangeForm.newJobName').reset();
		});
							...
							items : [{
								...
								listeners : {
									focus:function(combo){
									  var dep=Ext.getCmp('JobChangeForm.newDepId').getValue();
									  if(dep!=null&&dep!=''&&dep!='undefined'){
										  Ext.getCmp('JobChangeForm.newJobName').getStore().reload({
										     params :{
										        depId:dep
										     }
										  });
