				...
				},
				{
					...
					renderer : function(value, metadata, record, rowIndex,
						...
						if(isGranted('_StandSalaryDel')){
							str +='<button title="删除" value=" " class="btn-del" onclick="StandSalaryView.remove('
								+ editId + ')">&nbsp;&nbsp;</button>';
						}
						if(isGranted('_StandSalaryEdit')){
							str += '&nbsp;<button title="编辑" value=" " class="btn-edit" onclick="StandSalaryView.edit('
								+ editId + ')">&nbsp;&nbsp;</button>';
						}
						if(record.data.status != 1){//已经审核通过的不再审核
							if(isGranted('_StandSalaryCheck')){
								str += '&nbsp;<button title="审核" value=" " class="btn-salary-apply" onclick="StandSalaryView.check('
									+ editId + ')">&nbsp;&nbsp;</button>';
							}
						}
						if(isGranted('_StandSalaryQuery')){
							str += '&nbsp;<button title="操作纪录" value=" " class="btn-operation" onclick="StandSalaryView.operation('
								+ editId + ')">&nbsp;&nbsp;</button>';
						}
						return str;
					}
	...
	var store = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({
							...
							fields : [{
										name : 'standardId',
										type : 'int'
									}, 'standardNo', 'standardName',
									'totalMoney', 'setdownTime', 'status']
						}),
				remoteSort : true
			});
...
StandSalaryView.operation = function(_id) {
	...
	var window = new Ext.Window({
				...
				items : [new Ext.Panel({
							...
							autoLoad : {
								url : __ctxPath
										+ '/pages/hrm/standOperation.jsp?standardId='
										+ _id
							}
						})],
				buttons : [{
