									...
									handler : function() {
										UserSelector.getView(
												function(userId, fullname) {
													Ext.getCmp('empProfileForm.fullname').setValue(fullname);
													Ext.getCmp('empProfieldForm.userId').setValue(userId);
													Ext.Ajax.request({
														...
														method : 'post',
														success : function(response) {
															var result = Ext.util.JSON.decode(response.responseText).data[0];
															Ext.getCmp('empProfileForm.depId').setValue(result.department.depId);
															Ext.getCmp('empProfile.depName').setValue(result.department.depName);
														}
													})
												},true).show();
									}
																...
																.createUploadDialog(
																		{
																			...
																			callback : function uploadResumePhoto(
																					data) {
																				var photo = Ext
																						.getCmp('empProfileForm.photo');
																				var display = Ext
									...
									items : [{
										...
										listeners : {
											...
											select : function(combo, record,
													index) {
												Ext
														.getCmp('empProfileForm.jobId')
														.setValue(record.data.jobId);
											}
										}
									}, {
										fieldLabel : '薪酬标准单名称',
						...
						items : [{
							...
							items : [{
								...
								listeners : {
									focus : function(combo) {
										var _store = Ext
												.getCmp('empProfileForm.eduDegree')
												.getStore();
										if (_store.getCount() <= 0)
											Ext.Ajax.request({
												url : __ctxPath
								...
								listeners : {
									focus : function(combo) {
										var _store = Ext
												.getCmp('empProfileForm.eduMajor')
												.getStore();
										if (_store.getCount() <= 0)
											Ext.Ajax.request({
												url : __ctxPath
		...
		if (this.profileId != null && this.profileId != 'undefined') {
			this.formPanel.getForm().load({
				...
				success : function(form, action) {
					...
					if(res.startWorkDate != '' && res.startWorkDate !=null && res.startWorkDate !='undefined'){
						var startWorkDate = getDateFromFormat(res.startWorkDate,
							'yyyy-MM-dd HH:mm:ss');
							Ext.getCmp('empProfileForm.startWorkDate')
							.setValue(new Date(startWorkDate));
					}
					
					if(res.birthday != '' && res.birthday !=null && res.birthday !='undefined'){
						var birthday = getDateFromFormat(res.birthday,
								'yyyy-MM-dd HH:mm:ss');
						Ext.getCmp('empProfileForm.birthday').setValue(new Date(birthday));
					}
